...
import { userEditFormSchema, type UserEditFormValues } from "@/features/users/user-form-schema";
...
type UserEditDialogProps = {
  ...
  onSave: (values: UserEditFormValues) => void | Promise<void>;
};

function toDefaults(u: UserDto): UserEditFormValues {
  const names = splitFullName(u.fullName);
  const roleGroups = u.roleGroups ?? [];
  return {
    username: u.username,
    email: u.email,
...
export function UserEditDialog({
  ...
  useEffect(() => {
    ...
    for (const [field, message] of Object.entries(apiFieldErrors)) {
      form.setError(field as keyof UserEditFormValues, { type: "server", message });
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] max-w-2xl overflow-hidden">
        <DialogHeader>
          <DialogTitle>{ru.users.dialog.editTitle}</DialogTitle>
          <DialogDescription>{ru.users.dialog.editDesc}</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
          ...
          >
            ...
            <fieldset className="space-y-8 disabled:opacity-60" disabled={isSaving || !canSubmit || !user}>
              ...
              <section className="space-y-4">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{ru.users.form.securityEdit}</p>
                </div>
                <FormField
                  control={form.control}
                  name="password"
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel>{ru.users.form.passwordEdit}</FormLabel>
                      <FormControl>
                        <Input type="password" autoComplete="new-password" placeholder={ru.users.form.passwordPlaceholderEdit} {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                    ...
                    <FormItem>
                      <FormControl>
                        <AsyncGroupSelect
                          ...
                          preloadOnMount
                          initialSelected={(user?.roleGroups ?? []).map((group) => ({
                            id: Number(group.id),
                            code: group.code,
                            title: group.title
                          }))}
                          disabled={isSaving || !canSubmit || !user}
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
            ...
            <DialogFooter className="gap-2 sm:gap-0">
              ...
              <Button type="submit" disabled={isSaving || !canSubmit || !user}>
                {isSaving ? ru.users.dialog.saving : ru.users.dialog.saveChanges}
              </Button>
            </DialogFooter>
          </form>
        </Form>
